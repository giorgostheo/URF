...
def viz2(linestring, start_p, end_p):
    '''
    Vizualize the route alongisde the starting and the ending point. All routes will be in the same fig with the same color.
    '''
    m = gpd.GeoDataFrame([[i, LineString(line)] for i, line in enumerate([linestring])], columns=['id', 'geom'], geometry='geom', crs=4326).explore()
    m = start_p.buffer(0.001).explore(m=m, color='red')
    display(end_p.buffer(0.002).explore(m=m, color='black'))
    # input()
    # clear_output()
...
def vizpoints(points):
    '''
    Vizualize a list of points.
    '''
    m = gpd.GeoDataFrame([[i, point] for i, point in enumerate(points)], columns=['id', 'geom'], geometry='geom', crs=4326).explore(color='black')
    display(m)